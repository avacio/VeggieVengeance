...
bool Background::init(vec2 screen, GameMode mode, Stage stage)
{
	...
	unsigned int tutorialTextSize = 25;
	...
	left->setPosition({50.f, 260.f});
	right = new TextRenderer(mainFont, tutorialTextSize);
	right->setPosition({50.f, 290.f});
	crouch = new TextRenderer(mainFont, tutorialTextSize);
	crouch->setPosition({50.f, 320.f});
	reset = new TextRenderer(mainFont, tutorialTextSize);
	reset->setPosition({50.f, 350.f});
	pause = new TextRenderer(mainFont, tutorialTextSize);
	pause->setPosition({50.f, 380.f});
	ability1 = new TextRenderer(mainFont, tutorialTextSize);
	width = ability1->get_width_of_string("V/NUM2CommaSpecialAbility");
	ability1->setPosition({screen.x-(width*1.15f), 230.f});
	ability2 = new TextRenderer(mainFont, tutorialTextSize);
	width = ability2->get_width_of_string("B/NUM3PeriodSpecialAbility");
	ability2->setPosition({ screen.x - (width*1.15f), 260.f });

	shield = new TextRenderer(mainFont, tutorialTextSize);
	//width = shield->get_width_of_string("LShift/RShiftShield");
	//shield->setPosition({screen.x-(width*1.15f), 290.f});
	shield->setPosition({ 50.f, 470.f });
	punch = new TextRenderer(mainFont, tutorialTextSize);
	//width = punch->get_width_of_string("C/NUM1SlashPunch");
	//punch->setPosition({screen.x-(width*1.15f), 320.f});
	punch->setPosition({ 50.f, 440.f });
	pauseMusic = new TextRenderer(mainFont, tutorialTextSize);
	width = pauseMusic->get_width_of_string("Insert:PauseResumeSong");
	pauseMusic->setPosition({screen.x-(width*1.15f), 290.f});
	randomSong = new TextRenderer(mainFont, tutorialTextSize);
	width = randomSong->get_width_of_string("Home:RandomSong");
	randomSong->setPosition({screen.x-(width*1.15f), 350.f});
	changeSong = new TextRenderer(mainFont, tutorialTextSize);
	width = changeSong->get_width_of_string("Delete/End:PrevNextsong");
	changeSong->setPosition({ screen.x - (width*1.15f), 380.f});
	changeVolume = new TextRenderer(mainFont, tutorialTextSize);
	width = changeVolume->get_width_of_string("PageUpDow:nInc./Dec.volume");
	changeVolume->setPosition({screen.x-(width*1.15f), 320.f});
...
void Background::drawTutorialText(const mat3& projection) {
	...
	ability1->renderString(projection, "V/NUM2/Comma: Special Ability");
	ability2->renderString(projection, "B/NUM3/Period: Special Ability");
	shield->renderString(projection, "L Shift/R Shift: Block");
	punch->renderString(projection, "C/NUM1/Slash: Punch");
