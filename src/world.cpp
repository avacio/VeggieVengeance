...
void World::on_key(GLFWwindow *, int key, int, int action, int mod)
{
	...
	if (m_mode == MENU || m_mode == CHARSELECT)
	{
		...
		if (m_player1.get_in_play() && !m_paused && m_player1.get_alive())
		{
			...
			if (action == GLFW_RELEASE && (key == GLFW_KEY_B || key == GLFW_KEY_V) && !m_player1.broccoli_is_holding_cauliflowers())
		...
		if (m_player2.get_in_play() && !m_paused && m_player2.get_alive())
		{
			...
			if (action == GLFW_RELEASE && (key == GLFW_KEY_KP_3 || key == GLFW_KEY_COMMA) && m_player2.broccoli_is_holding_cauliflowers())
			...
			if (action == GLFW_RELEASE && (key == GLFW_KEY_KP_2 || GLFW_KEY_KP_3 || key == GLFW_KEY_PERIOD || key == GLFW_KEY_COMMA) && !m_player2.broccoli_is_holding_cauliflowers())
